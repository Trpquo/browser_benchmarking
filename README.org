...
Again, we first define the JavaScript function to catch user defined parameters for animation and then run functions for dots' array construction and animation rendering. In Canvas API (see [[https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API][MDN documentation]]), all elements are constructed virtually and then drawn onto ~<canvas>~ element, so the first task of our parent animation function is to create ~<canvas>~ element and place it onto our web pages staging area.
...
Next, the loop is created to calculate initial positions, colors and animation keyframes for all requested dot elements. Here, in contrast to CSS or JavaScript driven animations, we do not need to create any new DOM elements. 
...
Then all that is left is to define Canvas's rendering context and start the animation loop for each dot in an array.
...
And to clear Canvas' area one each animation loop start.
